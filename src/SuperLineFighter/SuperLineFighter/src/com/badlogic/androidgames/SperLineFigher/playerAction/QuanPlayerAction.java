...
public class QuanPlayerAction extends PlayerAction {

	private FighterObject fighterObject;
	
	Vector2 oriPosition = new Vector2();
	
	Const.GameElement gameElementQuan;
	double durTime = 0;
	int index = 0;
	double wordTime = 0;
	public QuanPlayerAction(FighterObject fighterObject){
	...
	public void update(double delTime) {
		...
		case 4:{
			gameElementQuan = new GameElement(fighterObject.position, 400, 100, 
					Assests.skillNameTextureRegionHashMap.get(Assests.SKILL.QUAN), 1);
			fighterObject.elementList.add(gameElementQuan);
			index = 5;
			wordTime = 0;
			break;
		}
		case 5:{
			...
			if(wordTime > ((double)1/showSpeed)){
				fighterObject.elementList.remove(gameElementQuan);
				index = 6;
			}
			wordTime += delTime;
			break;
