...
public class ShuaiPlayerAction extends PlayerAction {

	private FighterObject fighterObject;
	
	Vector2 oriPosition = new Vector2();
	
	Const.GameElement gameElementShuai;
	double durTime = 0;
	int index = 0;
	double wordTime = 0;
	
	public ShuaiPlayerAction(FighterObject fighterObject){
	...
	public void update(double delTime) {
		...
		case 4:{
			gameElementShuai = new GameElement(fighterObject.position, 400, 100, 
					Assests.skillNameTextureRegionHashMap.get(Assests.SKILL.SHUAI), 1);
			fighterObject.elementList.add(gameElementShuai);
			index = 5;
			wordTime = 0;
			break;
		}
		case 5:{
			...
			if(wordTime > ((double)1/showSpeed)){
				fighterObject.elementList.remove(gameElementShuai);
